...
public class Board {
    ...
    public void shiftLeft(){
        ...
        for (int tempX = 0; tempX<COLS; tempX++) {
            for (int tempY = 0; tempY <ROWS; tempY++) {
				temp[tempY][tempX] = board[tempY][(tempX + 1) % board[tempY].length];
    ...
    public void shiftRight(){
        int wew[][]	=new int[ROWS][COLS];
        //SHIFT RIGHT
         //kinukuha last ng column board and stores it in a temp board
                for (int tempX = COLS-1; tempX>0; tempX--) {
                    for (int tempY = ROWS-1; tempY >=0; tempY--) {
                        if(tempX+1!=COLS){
                            wew[tempY][tempX] = board[tempY][tempX-1];
                            
                        }
                        else{
                            wew[tempY][tempX] = board[tempY][5];
                            wew[tempY][0] = board[tempY][6];
                            
                        }
                    }
                }
                for (int i = 0; i < wew.length; i++) {
                    System.arraycopy(wew[i], 0, board[i], 0, wew[i].length);
                }
    ...
    public void shifting(int player_piece,int move_position){
        if(move_position==7){
            shiftRight();
            refreshRCD();
            return;
        }
        else if(move_position==8){
            shiftLeft();
            refreshRCD();
            return;
        }
